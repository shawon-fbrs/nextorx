...
async function main() {
  ...
  await prisma.$executeRawUnsafe(`TRUNCATE TABLE "Trade" CASCADE`);
  await prisma.$executeRawUnsafe(`TRUNCATE TABLE "DepositRequest" CASCADE`);
  await prisma.$executeRawUnsafe(`TRUNCATE TABLE "WithdrawalRequest" CASCADE`);
  await prisma.$executeRawUnsafe(`TRUNCATE TABLE "WalletHold" CASCADE`);
  ...
  await prisma.$executeRawUnsafe(`TRUNCATE TABLE "UserRiskProfile" CASCADE`);
  await prisma.$executeRawUnsafe(`TRUNCATE TABLE "PromoCodeUse" CASCADE`);
  await prisma.$executeRawUnsafe(`TRUNCATE TABLE "PromoCode" CASCADE`);
  await prisma.$executeRawUnsafe(`TRUNCATE TABLE "OtcCandle" CASCADE`);
  await prisma.$executeRawUnsafe(`TRUNCATE TABLE "OtcTick" CASCADE`);
  ...
  await prisma.$executeRawUnsafe(`TRUNCATE TABLE "Pair" CASCADE`);
  await prisma.$executeRawUnsafe(`TRUNCATE TABLE "PaymentMethod" CASCADE`);
  ...
  for (const pair of OTC_PAIRS) {
    await prisma.pair.create({ data: pair });
  }
