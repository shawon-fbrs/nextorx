...
export default function AuditClient({ initialData }: { initialData: AuditLog[] }) {
  const [logs] = useState<AuditLog[]>(initialData);
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map((log) => (
                    <tr key={log.id} className="border-b border-border/50 hover:bg-surface-hover/50">
                      ...
                      <td className="px-3 py-2 text-sm font-medium text-foreground">{log.actorName ?? 'System'}</td>
