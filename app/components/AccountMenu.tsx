...
export function AccountMenu({ balance, demoBalance = 0, realBalance, accountType, compact = false }: AccountMenuProps) {
  ...
  const accounts: Array<{ type: AccountType; label: string; color: string; disabled?: boolean }> = [
  ...
  return (
    <div className="relative">
      <button
        onClick={toggle}
        className={`flex items-center gap-3 bg-surface border border-border rounded-xl cursor-pointer hover:bg-surface-hover transition-colors ${compact ? 'pl-2.5 pr-2 h-10 gap-2' : 'pl-4 pr-3 h-10'}`}
      >
