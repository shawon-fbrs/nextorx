...
import { GraduationCap, Landmark, Shield, Trophy } from 'lucide-react';
...
export function AccountMenu({ balance, demoBalance = 0, realBalance, accountType, compact = false }: AccountMenuProps) {
  ...
  const accounts: Array<{ type: AccountType; label: string; color: string; icon: ReactNode; disabled?: boolean }> = [
    { type: 'demo', label: 'Demo', color: 'text-blue', icon: <GraduationCap className="w-5 h-5" /> },
    { type: 'real', label: 'Real', color: 'text-green', icon: <Landmark className="w-5 h-5" /> },
    { type: 'funded', label: 'Funded', color: 'text-orange', disabled: true, icon: <Shield className="w-5 h-5" /> },
    { type: 'tournament', label: 'Tournament', color: 'text-yellow', disabled: true, icon: <Trophy className="w-5 h-5" /> },
  ];
