'use client';

import Link from 'next/link';
import { useState, type ReactNode } from 'react';
import { usePathname, useRouter } from 'next/navigation';
import { useAuth } from '@/lib/auth-context';
import { LeaderboardDrawer } from './LeaderboardDrawer';
import { Podium, BanknoteArrowDown, BanknoteArrowUp, ArrowLeftRight, ChartSpline, TicketPercent } from 'lucide-react';

interface SidebarProps {
  expanded: boolean;
  onToggle: () => void;
}

function getAccountTypeFromPath(pathname: string): string {
  const match = pathname.match(/\/trade\/(\w+)/);
  return match ? match[1] : 'real';
}

const P = (d: string): ReactNode => (
  <svg className="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <path d={d} strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
  </svg>
);

const getNavItems = (accountType: string): Array<{ icon: ReactNode; label: string; href: string }> => [
  { icon: P('M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z'), label: 'Trade', href: `/trade/${accountType}` },
  { icon: P('M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z'), label: 'Support', href: '/support' },
  { icon: P('M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z'), label: 'Account', href: '/account' },
];

const getMoreItems = (): Array<{ icon: ReactNode; label: string; href: string }> => [
  { icon: <BanknoteArrowDown className="w-6 h-6 flex-shrink-0" />, label: 'Deposit', href: '/deposit' },
  { icon: <BanknoteArrowUp className="w-6 h-6 flex-shrink-0" />, label: 'Withdraw', href: '/withdraw' },
  { icon: <ArrowLeftRight className="w-6 h-6 flex-shrink-0" />, label: 'Transactions', href: '/transactions' },
  { icon: <ChartSpline className="w-6 h-6 flex-shrink-0" />, label: 'Analysis', href: '/analysis' },
  { icon: <TicketPercent className="w-6 h-6 flex-shrink-0" />, label: 'Bonus', href: '/bonus' },
  { icon: P('M12 4.354a4 4 0 110 7.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z'), label: 'Referrals', href: '/referrals' },
  { icon: P('M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9'), label: 'Notifications', href: '/notifications' },
];

export function Sidebar({ expanded, onToggle }: SidebarProps) {
  const pathname = usePathname();
  const router = useRouter();
  const { signOut } = useAuth();
  const [boardOpen, setBoardOpen] = useState(false);
  const accountType = getAccountTypeFromPath(pathname);
  const navItems = getNavItems(accountType);
  const moreItems = getMoreItems();

  return (
    <>
    <aside
      className={`bg-background border-r border-border flex flex-col justify-between flex-shrink-0 z-30 transition-all duration-300 ${
        expanded ? 'w-52' : 'w-[72px]'
      }`}
    >
      <div className="flex flex-col min-h-0 flex-1 overflow-hidden">
        <button
          onClick={onToggle}
          className="w-full h-16 flex-shrink-0 flex items-center justify-center hover:bg-surface transition-colors border-b border-border"
        >
          {expanded ? (
            <svg className="w-6 h-6 text-text-light" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path d="M6 18L18 6M6 6l12 12" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
            </svg>
          ) : (
            <svg className="w-6 h-6 text-text-light" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path d="M4 6h16M4 12h16M4 18h16" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
            </svg>
          )}
        </button>

        <div className={`flex flex-col pt-2 gap-1 overflow-y-auto flex-1 min-h-0 pb-4 ${expanded ? 'px-3' : 'items-center px-2'}`}>
          {navItems.map((item) => {
            const isActive = pathname === item.href || (item.href === '/support' && pathname === '/support');
            return (
              <Link
                key={item.label}
                href={item.href}
                title={expanded ? undefined : item.label}
                className={`flex items-center gap-3 rounded-xl transition-colors relative group ${
                  expanded ? 'px-4 py-2' : 'justify-center py-2.5 px-0 w-12'
                } ${
                  isActive
                    ? 'bg-blue/15 text-blue'
                    : 'text-text hover:bg-surface hover:text-foreground'
                }`}
              >
                {isActive && (
                  <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-blue rounded-r-md" />
                )}
                {item.icon}
                {expanded ? (
                  <span className="text-sm font-medium">{item.label}</span>
                ) : (
                  <span className="pointer-events-none absolute left-full ml-3 top-1/2 -translate-y-1/2 whitespace-nowrap bg-surface border border-border rounded-lg px-2.5 py-1.5 text-xs font-semibold text-foreground hidden group-hover:block z-50">
                    {item.label}
                  </span>
                )}
              </Link>
            );
          })}
          <button
            onClick={() => setBoardOpen(true)}
            title={expanded ? undefined : 'Leaderboard'}
            className={`flex items-center gap-3 rounded-xl transition-colors relative group w-full text-left ${
              expanded ? 'px-4 py-2' : 'justify-center py-2.5 px-0 w-12'
            } text-text hover:bg-surface hover:text-foreground`}
          >
            <Podium className="w-6 h-6 flex-shrink-0" />
            {expanded ? (
              <span className="text-sm font-medium">Leaderboard</span>
            ) : (
              <span className="pointer-events-none absolute left-full ml-3 top-1/2 -translate-y-1/2 whitespace-nowrap bg-surface border border-border rounded-lg px-2.5 py-1.5 text-xs font-semibold text-foreground hidden group-hover:block z-50">
                Leaderboard
              </span>
            )}
          </button>
          {expanded && (
            <span className="text-[10px] font-bold text-text-dark uppercase tracking-wider px-4 pt-2">Wallet</span>
          )}
          {moreItems.map((item) => {
            const isActive = pathname === item.href;
            return (
              <Link
                key={item.label}
                href={item.href}
                title={expanded ? undefined : item.label}
                className={`flex items-center gap-3 rounded-xl transition-colors relative group ${
                  expanded ? 'px-4 py-2' : 'justify-center py-2.5 px-0 w-12'
                } ${
                  isActive
                    ? 'bg-blue/15 text-blue'
                    : 'text-text hover:bg-surface hover:text-foreground'
                }`}
              >
                {isActive && (
                  <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-blue rounded-r-md" />
                )}
                {item.icon}
                {expanded ? (
                  <span className="text-sm font-medium">{item.label}</span>
                ) : (
                  <span className="pointer-events-none absolute left-full ml-3 top-1/2 -translate-y-1/2 whitespace-nowrap bg-surface border border-border rounded-lg px-2.5 py-1.5 text-xs font-semibold text-foreground hidden group-hover:block z-50">
                    {item.label}
                  </span>
                )}
              </Link>
            );
          })}
        </div>
      </div>

      <div className="flex flex-col items-center pb-6 gap-2">
        <button
          onClick={() => signOut().then(() => router.push('/login')).catch(() => router.push('/login'))}
          className="w-12 h-12 rounded-xl bg-surface border border-border text-red hover:bg-surface-hover flex flex-col items-center justify-center"
          title="Sign out"
        >
          <svg className="w-5 h-5 mb-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
          </svg>
          <span className="text-[9px] font-bold">Exit</span>
        </button>
      </div>
    </aside>
    <LeaderboardDrawer open={boardOpen} onClose={() => setBoardOpen(false)} />
    </>
  );
}
